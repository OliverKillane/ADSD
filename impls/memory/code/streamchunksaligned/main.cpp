...
int main() {
	...
	memcpy(source_data, (int8_t*)test_data, NUM_IMAGES * PADDED_IMAGE_SIZE);
	...
	for (int i = 0; i < 10; i++) {
		...
		XNonzero_Start(&mod);
		Xil_DCacheFlushRange((u32) source_data , NUM_IMAGES * PADDED_IMAGE_SIZE);
		Xil_DCacheFlushRange((u32) predictions, NUM_IMAGES);
		XAxiDma_SimpleTransfer(&dma, (u32) source_data , NUM_IMAGES * PADDED_IMAGE_SIZE, XAXIDMA_DMA_TO_DEVICE);
